...
func _physics_process(delta):
	if held:
		var forcePoint = pickupDir.rotated(rotation) + global_position
		...
		if(distance > stopDistance):
			...
			directionAndVelocityIndicator.global_position = forcePoint
			...
			if(predictedDir != dir):
				apply_force(dir * force * delta * (distance / distanceDecayFactor) * nonFacingCursorFactor, forcePoint)
			else:
				apply_force(dir * force * delta * (distance / distanceDecayFactor), forcePoint)
		else:
